...
- **Services**: `services/` - Business logic:
  ...
  - `cnbExchangeRate.ts` - CNB (Czech National Bank) exchange rate fetching with DB caching and weekend/holiday fallback. Converts EUR invoices and expenses to CZK for dashboard totals and paušální daň tracking
...
- **Components**: `components/` - Reusable UI:
  ...
  - `MobileBottomNav.tsx` - Mobile bottom tab bar (Dashboard, Invoices, new-invoice action, Clients, More sheet with secondary navigation, AI assistant, appearance toggle, and logout). The sheet has no Profile entry — the mobile settings index is the only way into it. The sheet is dismissable by dragging it down (or tapping the grabber): it tracks the finger, the scrim fades with the pull, and it closes on a long pull or a fast flick, springing back otherwise
  ...
  - `StickySaveBar.tsx` - Save/discard bar used by Settings and Profile. Hidden until the form is dirty (current state compared against the last saved state), then fixed to the bottom of the viewport above the mobile tab bar. Uses `fixed`, not `sticky`, because Layout's content wrapper has `overflow-x-hidden`, which disables sticky inside it
  - `SettingsList.tsx` - Grouped settings-list primitives (`SettingsGroup` card with inset dividers, `SettingsRow` with icon tile / trailing value / toggle, `StatusValue`, `RowToggle`, `SettingsBackHeader`) used by the mobile Settings and Profile indexes
...
- **Pages**: `pages/` - Dashboard, Clients, ClientDetail, Invoices, InvoiceCreate, InvoiceDetail, RecurringInvoices, RecurringInvoiceCreate, RecurringInvoiceDetail, Expenses, ExpenseCreate, ExpenseDetail, Payments, Settings, Profile, Calculator, Login, Register, Onboarding, ForgotPassword, ResetPassword
  - `Settings.tsx` / `Profile.tsx`: sections are routes (`/settings/:section` — invoicing, email, bank-matching, ai, calculator, appearance, language; `/profile/:section` — personal, company, bank, logo, password, danger), not local state, so mobile drills in and browser Back works. Desktop keeps the two-column nav + panel at every route; below `lg` the section-less route renders a grouped index instead. The Settings index is the single mobile entry point into settings *and* profile; Language moved from Profile to `/settings/language` and applies immediately (like Appearance), sending the whole profile because `PUT /auth/me` overwrites company fields
  - `Dashboard.tsx` revenue chart: hand-rolled bar pairs (no chart library). Months are buttons: hovering shows the tooltip, tapping pins it open (a pin outranks the pointer; dismissed by a second tap, a tap outside the plot, Escape, or a year change). The plot is `select-none` + `-webkit-touch-callout:none` so a finger resting on a month is not taken as a long-press. The tooltip carries the month's Income, Expenses and Net and is anchored in pixels off a measured wrapper so the edge months stay inside the card; it uses `bg-text`/`text-canvas`, which invert together, so it reads in both themes. The year picker runs oldest→newest with the latest on the right and shows three years at a time, chevrons page further back. The flat-rate tax footnote follows the selected year — a pace projection for the running year, a final in/over verdict for a year that has closed
- **Utils**:
...
- **CNB Exchange Rates**: Auto-fetches daily EUR/CZK rates from the Czech National Bank. EUR invoices and expenses store `exchange_rate` and `total_czk` for accurate dashboard totals and paušální daň tracking (`services/cnbExchangeRate.ts`)
